...
def variants():
    ...
    for word, path in STEEL_ITEMS:
        for mark, facet in STEELS.items():
            ...
            if not slug:
                continue
            # Голая марка-число («труба 20») при широком соответствии читается как
            # диаметр 20 мм: 11.09.2026 так пришёл запрос про полимерную трубу
            # varmega PE-Xa. Для числовых марок — только со словом «сталь».
            tpls = ((f"{word} из стали {mark}", f"{word} сталь {mark}") if mark.isdigit()
                    else (f"{word} {mark}", f"{word} из стали {mark}", f"{word} сталь {mark}"))
            for tpl in tpls:
