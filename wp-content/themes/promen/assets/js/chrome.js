...
(function () {
  ...
  /* S10 · наезд на десктопе (>1024): считаем «посадочную полосу».
     ─────────────────────────────────────────────────────────────────
     Футер должен заехать на секцию ровно на её пустой хвост — от низа
     последней строки контента до низа секции — и там остановиться.
     Дальше пин отпускается (sticky-контекст кончается на .s10-track),
     и форма уезжает вместе с футером, а кромка держится под контентом.

     Хвост нельзя зашить константой: .s10-inner{min-height:100vh}, значит
     на экране 1080 хвост ~300px, на 900 — ~130px, а на 700 секция уже
     ВЫШЕ вьюпорта и хвоста нет вовсе. Поэтому меряем по факту, а на
     низких экранах дополнительно якорим пин низом (top < 0), иначе
     кнопка «Отправить» остаётся за нижним краем и недостижима: липкий
     элемент выше вьюпорта прокруткой не сдвинуть. */
  var s10Sec = document.querySelector('.footer-zone .s10');
  var s10Track = document.querySelector('.s10-track');
  var s10Zone = document.querySelector('.footer-zone');
  if (s10Sec && s10Track && s10Zone) {
    var mqDesk = window.matchMedia('(min-width:1025px)');
    var LAND_GAP = 14;   // воздух между кромкой и последней строкой контента
    var LAND_MAX = 200;  // на очень высоком экране хвост огромен — не весь он наш

    /* Колонки растянуты на всю высоту секции, поэтому их собственный rect
       кончается по низу секции и для замера бесполезен — берём низ самого
       нижнего видимого потомка. */
    var contentBottom = function (col) {
      var max = 0, i, r;
      if (!col) return max;
      for (i = 0; i < col.children.length; i++) {
        if (col.children[i].getClientRects().length === 0) continue;
        r = col.children[i].getBoundingClientRect();
    ...
    var setLanding = function () {
      if (!mqDesk.matches) {
        s10Zone.style.removeProperty('--s10-overlap');
        s10Sec.style.top = '';
        s10Track.style.height = '';
        return;
      }
      /* Секция выше экрана — пиним низом к нижнему краю. */
      s10Sec.style.top = Math.min(0, window.innerHeight - s10Sec.offsetHeight) + 'px';

      var secBottom = s10Sec.getBoundingClientRect().bottom;
      var lowest = Math.max(
        contentBottom(s10Sec.querySelector('.s10-left')),
        contentBottom(s10Sec.querySelector('.s10-right'))
      );
      var tail = lowest ? secBottom - lowest - LAND_GAP : 0;
      var overlap = Math.max(0, Math.min(LAND_MAX, Math.round(tail)));
      s10Zone.style.setProperty('--s10-overlap', overlap + 'px');

      /* Ход пина = пауза + наезд. Сначала форма стоит неподвижно (pause),
         потом тёмный край поднимается по хвосту (overlap) и встаёт под
         контентом — и только тогда пин отпускается. Именно высотой, а не
         паддингом: см. комментарий в base.css. */
      var pause = Math.round(window.innerHeight * 0.15);
      s10Track.style.height = (s10Sec.offsetHeight + pause + overlap) + 'px';
    };

    setLanding();
    window.addEventListener('resize', setLanding, { passive: true });
    window.addEventListener('load', setLanding);
    if (mqDesk.addEventListener) mqDesk.addEventListener('change', setLanding);
    /* Высота формы меняется без ресайза окна: пришло сообщение об отправке,
       подставилось длинное имя файла, дорисовался шрифт. */
    if (window.ResizeObserver) new ResizeObserver(setLanding).observe(s10Sec);
  }
