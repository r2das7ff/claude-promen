...
function promen_rest_delivery_quote( WP_REST_Request $request ) {
	...
	if ( ( $dated['variant'] ?? '' ) === 'address' ) {
		// При заборе от адреса produceDate требует интервал передачи груза.
	...
	// Дата отправки могла попасть на праздник — пробуем ещё раз без неё
	// (цена посчитается, график движения просто не покажем).
	if ( ! is_wp_error( $json )
	...
	// Срок: дата готовности к выдаче на терминале-получателе.
