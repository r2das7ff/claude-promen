...
add_filter( 'document_title_parts', function ( array $parts ): array {
	...
	if ( is_singular() && ! is_front_page() ) {
		 ...
		 *
		 * В <title> уходило имя страницы из админки: «Производство», «Проекты»,
		 * «Контакты» — 17–26 символов из шестидесяти видимых в выдаче, без
		 * предмета и без отрасли. Обход 25.09.2026 нашёл тринадцать таких.
		 * Особенно обидно на /production/: страница стоит первой по запросу
		 * «производство деталей тэс и аэс», и совпадение идёт по H1 и тексту,
		 * а сам заголовок этих слов не содержит.
		 *
		 * Тексты повторяют H1 страницы, укороченный до места в выдаче: так
		 * заголовок и первый экран говорят одно и то же.
		 */
		$by_slug = [
			'production'        => 'Производство деталей трубопроводов для АЭС и ТЭС',
			'proekty'           => 'Проекты: поставки на АЭС, ТЭС и ГРЭС',
			'contacts'          => 'Контакты завода: Челябинск, отдел продаж',
			'kalkulyatory'      => 'Калькуляторы и справочники по ГОСТ и ОСТ',
			'normativnaya-baza' => 'Нормативная база: ГОСТ, ОСТ, СТО ЦКТИ и ТУ',
			'podbor'            => 'Подбор детали трубопровода по параметрам',
			'stati'             => 'Статьи о деталях трубопроводов и марках стали',
			// Объекты: различаются головой, хвост общий — это серия страниц.
			'kurskaya-aes'                 => 'Курская АЭС‑2 — поставка деталей трубопроводов',
			'aes-akkuyu'                   => 'АЭС «Аккую» — поставка деталей трубопроводов',
			'aes-ruppur'                   => 'АЭС «Руппур» — поставка деталей трубопроводов',
			'cherepetskaya-gres'           => 'Черепетская ГРЭС — поставка деталей трубопроводов',
			'teploelektrocentral-tec-3'    => 'Омская ТЭЦ‑3 — поставка деталей трубопроводов',
		];
...
/*
 * Закрытие стенда после переезда (2026-09-01).
 *
 * Стенд отдаёт тот же контент, что и prom-en.com, и был открыт для
 * индексации. Два одинаковых сайта — риск склейки не в ту сторону:
 * канонической Яндекс может выбрать тестовый домен, и весь накопленный
 * вес уйдёт туда. Поэтому весь стенд — 301 на прод, адрес в адрес:
 * структура URL одинаковая, значит редирект ведёт на смысловой аналог,
 * а не в корень, и вес передаётся постранично.
 *
 * robots.txt НЕ редиректим намеренно. Робот сначала читает его, и если
 * там окажется редирект или запрет обхода, он не пойдёт по страницам —
 * а значит не увидит 301 и не выкинет стенд из индекса. Чтобы дубль
 * ушёл, обход должен остаться разрешённым.
 */
add_action( 'template_redirect', function () {
	if ( 'prom-en.forgotaboutdre.ru' !== ( $_SERVER['HTTP_HOST'] ?? '' ) ) {
		return;
	}
	if ( is_robots() || is_admin() || ( defined( 'DOING_CRON' ) && DOING_CRON ) ) {
		return;
	}
	wp_redirect( 'https://prom-en.com' . (string) ( $_SERVER['REQUEST_URI'] ?? '/' ), 301 );
	exit;
}, 0 );
...
/**
 * Переименованные и склеенные нормативы: 301 со старого слага на новый.
 *
 * Слаг норматива встречается в двух видах адресов — архив /normativy/<слаг>/
 * и страница серии /catalog/<путь>/seriya/<слаг>[-<угол>]/. При склейке
 * терминов оба адреса иначе просто отдали бы 404, хотя содержимое никуда не
 * делось, а перешло к выжившему термину.
 *
 * ost-34-10-432-1990 → ost-34-10-432-90: два термина на один документ,
 * у каждого по пять тройников одних типоразмеров. Верным признан слаг с
 * двузначным годом — реестр (normatives/registry/normatives_master.csv)
 * записывает семейство как «ОСТ 34-10-416-90 ÷ ОСТ 34-10-433-90», а у
 * выжившего термина товары полнее: с исполнением и маркой 08Х18Н10Т.
 */
...
add_action( 'template_redirect', function (): void {
	// Без проверки на 404 намеренно: адрес серии со снесённым нормативом до
	// честной 404 не доходит — маршрут не находит репрезентативный товар и
	// отдаёт запрос дальше. А слаги в карте ниже уже не существуют нигде,
	// так что живой адрес под правило не попадёт.
	$path = (string) parse_url( $_SERVER['REQUEST_URI'] ?? '', PHP_URL_PATH );
	if ( '' === $path ) {
		return;
	}
	foreach ( promen_norm_slug_redirects() as $old => $new ) {
		// Слаг — отдельный сегмент адреса либо основа сегмента серии с углом.
		$re = '#/' . preg_quote( $old, '#' ) . '(-\d{2,3})?/#';
		if ( preg_match( $re, $path ) ) {
			$target = preg_replace( $re, '/' . $new . '$1/', $path );
...
/*
 * Блоговые архивы — честная 404, а не пустая страница с noindex.
 *
 * Блога на сайте нет: статьи живут страницами в /stati/, таксономия category
 * не использовалась ничем, кроме дефолтной записи «Hello world!». Её удалили
 * 25.09.2026, и /category/uncategorized/ осталась отвечать 200 без единого
 * материала — это мягкая 404, которую поисковик считает сам и не любит.
 * Архивы автора и дат там же: содержимого у них нет по определению.
 *
 * Приоритет 11 — после рендерера ядра, как у карты сайта выше.
 */
add_action( 'template_redirect', function (): void {
...
function promen_series_sitemap_urls(): array {
	...
	// У какой пары «категория + норматив» встречается угол.
	//
	// У отводов угол есть всегда, и безугловой адрес серии оказывается копией
	// углового: /seriya/gost-17375-2001/ отдаёт то же, что
	// /seriya/gost-17375-2001-60/, с тем же title. Канон это разводит — он
	// ведёт на угловой, — но в карте сайта неканоническим адресам не место.
	// Обход 25.09.2026 нашёл 17 таких пар из 200 адресов карты.
	$has_angle = [];
	foreach ( (array) $rows as $r ) {
		if ( '' !== (string) $r['angle'] ) {
			$has_angle[ $r['cat'] . '|' . $r['norm'] ] = true;
		}
	}
	...
	foreach ( (array) $rows as $r ) {
		...
		if ( '' === (string) $r['angle'] && ! empty( $has_angle[ $cat . '|' . $r['norm'] ] ) ) {
			continue; // безугловой дубль углового
		}
