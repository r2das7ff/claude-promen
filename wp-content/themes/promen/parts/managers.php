 ...
 *
 * 2026-08-27: из состава выведены Виктор Саитгареев и Сергей Беляков;
 * Сергей Курбатов переведён на место Виктора в атомный департамент со своими
 * прямыми линиями (доб. 207, zakaz7@). Фото выбывших оставлены в assets.
 *
 ...
 * назначение») снято — один список специалистов без полос-заголовков.
 ...
 * команды.
 *
 * 2026-09-02: из состава выведен Кирилл Белов, порядок остальных задан
 * заказчиком (Калашников, Романов, Курбатов, Хадеев, Отвагина, Меньшиков).
 * Шесть человек — это ровно два ряда по три, сетка закрывается без хвоста.
 * Фото и ролик Белова оставлены в assets, как у Саитгареева и Белякова:
 * возврат в состав — одна строка массива.
 */
...
// Порядок задан заказчиком и содержателен: он определяет, кого клиент увидит
// первым, поэтому произвольной пересортировке (по алфавиту, по добавочному)
// список не подлежит.
$smgr_managers = [
          ...
          <div class="smgr-photo<?php echo $smgr_vid ? ' has-video' : ''; ?>">
            ...
            <?php if ( $smgr_vid ) : ?>
              ...
              <video class="smgr-video" muted playsinline preload="none" aria-hidden="true" tabindex="-1"<?php echo $smgr_zoom ? ' style="' . esc_attr( $smgr_zoom ) . '"' : ''; ?>>
                <?php /* Отдаём только те форматы, что реально лежат. У Курбатова
                         webm не собирается: на мелком рисунке ткани (полоска)
                         VP9 даёт пульсацию и рывки, а H.264 декодируется
                         аппаратно и идёт ровно. */ ?>
