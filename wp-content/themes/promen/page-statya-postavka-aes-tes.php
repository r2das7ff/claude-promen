...
<div class="pg">
  ...
  <!-- MEDIA -->
  <div class="ar-media">
    <img src="<?php echo esc_url( get_theme_file_uri( 'assets/img/photos/promen-photo-hor-3.jpg' ) ); ?>" alt="Отгрузка партии деталей трубопровода" loading="eager">
  ...
  <div class="ar-split">
    <article class="ar-content">
      ...
      <p>Для объектов атомной энергетики — как Курская АЭС‑2, «Руппур» или «Аккую» — определяющее требование не столько сама деталь, сколько её сопроводимая история: от сертификата на металл и номера плавки до полного протокола неразрушающего контроля каждого шва. Это тот же принцип прослеживаемости, что описан в статье <a href="<?php echo esc_url( promen_article_url( 'kontrol-kachestva' ) ); ?>" style="color:var(--dark);border-bottom:1px solid var(--g1);">«Контроль качества СДТ»</a> — но применяется без исключений и с расширенным объёмом проверок.</p>
      ...
      <p>И для АЭС, и для ТЭС применяется государственная и отраслевая нормативная база — ГОСТ и ОСТ. Но для атомных объектов к ней добавляются нормы и правила ПНАЭ и корпоративные СТО заказчика (например, Росатома), которые задают собственные требования к контролю и документообороту. Подробно эта иерархия разобрана в статье <a href="<?php echo esc_url( promen_article_url( 'normativnaya-baza' ) ); ?>" style="color:var(--dark);border-bottom:1px solid var(--g1);">«ГОСТ, ОСТ и ТУ на СДТ»</a>.</p>

      <h2 id="s5"><span class="num">05</span>Как начинается поставка на ваш объект</h2>
      <p>Независимо от класса объекта, порядок работы одинаков: инженер завода анализирует параметры трубопровода и применимый нормативный документ, подбирает материал и технологию, после чего фиксирует объём контроля и состав пакета документов в спецификации. Для нестандартной геометрии этот процесс начинается с чертежа заказчика — см. статью <a href="<?php echo esc_url( promen_article_url( 'chertezh-zakazchika' ) ); ?>" style="color:var(--dark);border-bottom:1px solid var(--g1);">«Изготовление по чертежам заказчика»</a>.</p>
      <p>Полный список реализованных проектов с подробной историей поставки — на странице <a href="<?php echo esc_url( $promen_proekty_url ); ?>" style="color:var(--dark);border-bottom:1px solid var(--g1);">«Проекты»</a>.</p>
