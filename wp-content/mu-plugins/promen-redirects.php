...
add_action( 'init', function () {
	...
	/*
	 * Карта сайта. В Яндекс.Вебмастере у домена зарегистрирован СТАРЫЙ адрес
	 * /sitemap.xml (10 174 URL, статический файл, собранный Netpeak Spider).
	 * На новом сайте карта штатная, /wp-sitemap.xml. Без этого правила
	 * зарегистрированный источник адресов начал бы отдавать 404, и робот
	 * остался бы без списка страниц ровно в тот момент, когда он ему нужнее
	 * всего — сразу после смены структуры URL.
	 */
	...
	$target = $map[ $key ] ?? '';
	// Пагинация старых рубрик (/rubric-products/<slug>/page/5/): в карте только
	// сама рубрика, и такие адреса отдавали 404 — 399 раз за 16–24.09. Ведём
	// туда же, куда рубрику: страница 5 старой выдачи — это та же категория.
	if ( '' === $target && preg_match( '~^(/rubric-products/.+/)page/\d+/$~', $key, $m ) ) {
		$target = $map[ $m[1] ] ?? '';
	}
	if ( '' === $target ) {
