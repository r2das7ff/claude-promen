...
После `cp` открой `.env` и заполни `PROMEN_DELLIN_APPKEY` — ключ API
«Деловых Линий» для калькулятора доставки. Действующее значение лежит в
`site/.env` OneDrive-копии проекта (между машинами его переносит OneDrive,
в git его нет); первоисточник — письмо после регистрации на dev.dellin.ru.
Без ключа стек поднимется и всё будет работать, кроме одного: кнопка
«Рассчитать доставку» на карточке товара молча превратится в обычную форму
запроса (`promen_delivery_available()` вернёт false, ошибок не будет).
...
## 4. WooCommerce — обязательно, не пропускай

Это главная ловушка развёртывания. WooCommerce живёт в named volume
`wpcore`, а не в репозитории, и на новой машине volume создаётся пустым.
При этом в дампе плагин помечен активным и все 20 таблиц `wp_wc*` на
месте — сайт отдаёт 200 и выглядит полностью рабочим. Но функции
`wc_get_product()` не существует, и следующий шаг падает с фаталом.

```bash
docker compose run --rm wpcli plugin install woocommerce --version=10.9.4
```

Версию бери строго из базы, а не последнюю: более свежая прогонит
апгрейд схемы по восстановленному каталогу.

```bash
docker compose exec -T db mariadb -uwp -pwp wordpress -N -e "SELECT option_value FROM wp_options WHERE option_name='woocommerce_db_version';"
```

Если число разошлось с `10.9.4` — ставь то, что вернула база.
...
**`curl` здесь годится только на код ответа и поиск фаталов.** Реестры
каталога и нормативной базы рендерятся на клиенте: в исходном HTML стоят
плейсхолдеры `—` и ноль ссылок на товары даже на полностью исправной
странице. Пустой реестр в `curl` — не признак поломки, и наоборот: 200 в
ответе ничего не говорит о наполнении.

Наполнение, счётчики и фасеты проверяй браузером с исполнением JS:
...
| `/production/` | карта цеха, лента участков, видео |
| карточка любого товара | паспорт, марки стали, «Запросить КП» |

Ориентир по данным: 15 407 опубликованных товаров и столько же строк в
каноне `wp_promen_catalog_rows`, drift 0. Счётчики на живых страницах
должны сходиться с этим до единицы.
